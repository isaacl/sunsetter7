...
bitboard boardStruct::attacksTo(square sq)
   ...
   attacks = attacksFrom(KNIGHT, sq) & pieces[KNIGHT];
   attacks |= pawnAttacksFrom(WHITE, sq) & pieces[PAWN] & occupied[BLACK];
   attacks |= pawnAttacksFrom(BLACK, sq) & pieces[PAWN] & occupied[WHITE];
   attacks |= attacksFrom(BISHOP, sq) & (pieces[QUEEN] | pieces[BISHOP]);
   attacks |= attacksFrom(ROOK, sq) & (pieces[QUEEN] | pieces[ROOK]);
